...
class JointEventPlanModel(nn.Module):
    def __init__(self,d=256):
        ...
        self.intent=nn.ModuleList([nn.Sequential(nn.LayerNorm(d),nn.Linear(d,d),nn.GELU()) for _ in range(5)])
        self.arity=nn.ModuleList([nn.Linear(d,3) for _ in range(5)]);self.stars=nn.ModuleList([nn.Linear(d,3) for _ in range(5)])
        self.holds=nn.ModuleList([nn.Linear(d,3) for _ in range(5)]);self.touches=nn.ModuleList([nn.Linear(d,34) for _ in range(5)])
        self.tap_slide=nn.ModuleList([nn.Linear(d,1) for _ in range(5)])
    def forward(self,audio,structure,version,levels):
        ...
        for i,slot_index in enumerate(range(5)):
